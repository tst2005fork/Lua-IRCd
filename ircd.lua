...
local function ircWrite(clientInfo, text)
	local nick = ""
...
local function Command_TOPIC(clientInfo, args)
...
local function Command_NAMES(clientInfo, channelName)
...
local function Command_JOIN(clientInfo, channels)
...
local function Command_LIST(clientInfo, args)
	local target
	if args then
...
local function Command_MODE(clientInfo, args)
...
local function Command_NICK(clientInfo, nick)
...
local function Command_NOTICE(clientInfo, args)
	if not args then return end
...
local function Command_PING(clientInfo, message)
	SendRawMessage(clientInfo, "PONG " .. config.hostname .. " " .. message)
end


local function Command_PONG(clientInfo, message)
end


local function Command_PRIVMSG(clientInfo, args)
	if not args then return end
	local _, _, target, message = args:find("^([^ ]+) +:(.*)$")
	if not target then return end
...
local function Command_WHO(clientInfo, args)
...
local function Command_WHOIS(clientInfo, nick)
	if not nick then return end
	...
	if targetInfo then
		...
		SendServerClientMessage(clientInfo, 312, nick .. " " .. config.hostname .. " :" .. config.hostname)
	end
...
local function ProcessClient(clientInfo)
	...
	if clientInfo.Nick and clientInfo.User then
		...
		SendServerClientMessage(clientInfo, 001, "Welcome to the LuaIRC server " .. clientInfo.Nick .. "!" .. clientInfo.User .. "@" .. clientInfo.Host)
		SendServerClientMessage(clientInfo, 002, "Your host is " .. config.hostname .. ", running version " .. config.version)
		SendServerClientMessage(clientInfo, 003, "This server was created ...")
		SendServerClientMessage(clientInfo, 004, config.hostname .. " " .. config.version .. " aAbBcCdDeEfFGhHiIjkKlLmMnNopPQrRsStUvVwWxXyYzZ0123459*@ bcdefFhiIklmnoPqstv")
...
local function RunServer()
	io.write("Opening server ("..config.hostname..":"..config.port..") ...\n")
		...
		for _, input in ipairs(readable) do
			...
			if input == server then
				...
				if new then
					...
					SendRawMessage({ Client = new }, "NOTICE AUTH :" .. config.version .. " initialized.")
